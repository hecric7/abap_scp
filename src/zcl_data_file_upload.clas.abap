...
CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS: get_input_field_value IMPORTING name TYPE string dataref TYPE data RETURNING VALUE(value) TYPE string.
...
* Check file extension is valid, only json today.
        IF fileext <> 'json'.
          response->set_status( i_code = if_web_http_status=>bad_request i_reason = 'File typenot supported' ).
